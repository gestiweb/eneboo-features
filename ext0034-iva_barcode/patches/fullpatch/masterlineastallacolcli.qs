...
function interna_init()
{
	...
	var tablaPadre:String = cursor.valueBuffer("tabla").right(cursor.valueBuffer("tabla").length - 6);

	var campoPadre:String  = cursor.valueBuffer("campopadre");
	var valorCampoPadre:String = cursor.valueBuffer("valorcampopadre");

	var codSerie:String = util.sqlSelect(tablaPadre, "codserie", campoPadre + " = " + valorCampoPadre);
	var codCliente:String = util.sqlSelect(tablaPadre, "codcliente", campoPadre + " = " + valorCampoPadre);
	...
	while (qryLineas.next()) {
		...
		if (cursor.valueBuffer("tabla") == "lineasalbaranescli") {
			if (qryLineas.value("COUNT(idlinea)") > 1) {
				if (util.sqlSelect("lineasalbaranescli", "idlinea", "idalbaran = " + cursor.valueBuffer("valorcampopadre") + " AND barcode = '" + arrayTC[numFilas][numColumnas]["barcode"] + "' AND idlineapedido <> 0 AND idlineapedido IS NOT NULL")) {
...
function oficial_insertar()
{
	...
	var referencia:String = cursor.valueBuffer("referencia");
	var pvpInicial:Number = parseFloat(cursor.valueBuffer("pvpunitario"));
	pvpInicial = util.roundFieldValue(pvpInicial, "lineastallacolcli", "pvpunitario");
	var texto:String;
	var codTalla:String;
	var codColor:String;
	var barCode:String;
	...
	for (var fila:Number = 0; fila < this.iface.tablaLineasTC.numRows(); fila++) {
		for (var columna:Number = 0; columna < this.iface.arrayLineasTC[fila].length; columna++) {
			...
			pvp = util.roundFieldValue(pvp, "lineastallacolcli", "pvpunitario");
			if (preguntar && pvp != pvpInicial) {
				...
				texto.text = util.translate("scripts", "El precio del barcode %1 (talla %2, color %3) %4 es distinto del precio del artículo %5\n¿Qué desea hacer?").arg(barCode).arg(codTalla).arg(codColor).arg(pvpInicial).arg(pvp);
				dialog.add( texto );
				
				var gbxOpciones = new GroupBox;
				var rbnOp1 = new RadioButton;
				rbnOp1.text = util.translate("scripts", "Usar el coste indicado (%1)").arg(pvpInicial);
				rbnOp1.checked = true;
				gbxOpciones.add(rbnOp1);

				var rbnOp2 = new RadioButton;
				rbnOp2.text = util.translate("scripts", "Usar el precio del artículo (%1)").arg(pvp);
